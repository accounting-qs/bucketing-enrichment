...
function DataCell({ value }: { value: any }) {
    ...
    return (
        <td className="px-6 py-4 text-slate-700 dark:text-slate-300 font-medium truncate max-w-[250px] group-hover/row:text-primary relative group/cell">
            {isUrl ? (
                <a href={href} target="_blank" rel="noopener noreferrer" className="text-emerald-500 hover:text-emerald-600 hover:underline relative z-10">
                    {val}
                </a>
            ) : (
                val
            ...
            <div className="absolute left-6 top-[calc(100%-12px)] opacity-0 invisible group-hover/cell:opacity-100 group-hover/cell:visible transition-all z-[60] shadow-xl scale-95 group-hover/cell:scale-100 origin-top-left pointer-events-none">
                <div className="bg-slate-900 text-slate-100 text-[11px] font-bold px-3 py-2 rounded-lg whitespace-normal break-words max-w-xs border border-slate-700 leading-relaxed shadow-xl">
                    {val}
                </div>
            </div>
        </td>
